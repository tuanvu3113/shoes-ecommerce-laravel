    ...
    MultipleSelect.prototype = {
        ...
        init: function() { //console.log(this);
            ...
            if (this.options.selectAll && !this.options.single) {
				...
				html.push(
					'<li>',
					   '<label style="color:#169ef4">',
							'<input type="checkbox" name="selectAll" class="'+this.options.allSelected+'" /> ',
							this.options.selectAllText,
						'</label>',
					'</li>'
				);
				/*if(this.options.placeholder == '<i class="fa fa-map-marker"></i>Locations'){
					html.push(
						'<li>',
						   '<label style="color:#169ef4">',
								'<input type="checkbox" name="selectAll" class="'+this.options.allSelected+'" /> ',
            ...
            if ($elm.is('option')) {
                ...
                html.push(
                    '<li' + (multiple ? ' class="multiple"' : '') + '>',
                        '<label' + (disabled ? ' class="disabled option"' : ' class="option"') + '>',
        ...
        updateSelectAll: function() {
			...
			//console.log($items.filter(':checked').length);
			if(this.$el.context.id == "incoming-volume-by-user-user"){
				if($items.length == $items.filter(':checked').length){
					checkAllDataPerUser = true; //alert('checkall');
				}
				else{
					checkAllDataPerUser = false;
				}
			}
			if(this.$el.context.id == "failure-per-user"){
				if($items.length == $items.filter(':checked').length){
					checkAllDataFailureUser = true; //alert('checkall');
				}
				else{
					checkAllDataFailureUser = false;
				}
			}
			
			if(this.$el.context.id == "location"){
				if($items.length == $items.filter(':checked').length){
					//alert('checkall');
				}
				else{
        ...
        checkAll: function() {
			if(typeof this.$selectItems == 'object'){
        ...
        uncheckAll: function() {
			...
			*/
			if(this.$el.context.id == "incoming-volume-by-user-user"){
				checkAllDataPerUser = false; //alert('uncheckall');
			}
			if(this.$el.context.id == "failure-per-user"){
				checkAllDataFailureUser = false; //alert('checkall');
			}
			if(this.$el.context.id == "location"){
