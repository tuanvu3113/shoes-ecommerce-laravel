Optimize = {
	initFormControl: function(){
		$(".search").each(function(){
			...
			if(optsl != undefined){
				...
				if(configSelect.onClick != undefined){
					$( "input[name*='selectAll"+name+"']" ).live('click', function(){ 
		...
		$("#"+gridView+" tr td").live('click',function(){
			if(
				...
				oldLocId = $('#location_id').multipleSelect('getSelects');
			...
			if(optsl != undefined){
				...
				if(val == "''" || val == '' || typeof val == 'object'){
					if(optslCheckAllForSearch != undefined){
						if(optsl === 'single') {
							var myOptions = $('#'+name +' option');
							var myValues = $.map(myOptions ,function(option) {
								return option.value;
							});							
							val = myValues.join(',');
						} else {
			...
			success: function(data) {
				...
				$('.delete').each(function(){
					$(this).click(function(){
						...
						if(typeof(check_Delete)=='function'){
							check_Delete(idDel);
						}
						else{
							$.msgBox({
		...
		$.ajax({
			...
			success:function(data){
				...
				$.msgBox({
					...
					buttons: [{ value: "OK"}],
					success: function (result) {
						if(data.indexOf('success') >= 0 || data.indexOf('Successfully') >= 0){
	...
	saveDataWithAjax: function(id, dataPost,title){
		var el = jQuery("#"+gridView);
		var elFS = jQuery("#"+formView);
		App.blockUI({target: el, iconOnly: true});
        App.blockUI({target: elFS, iconOnly: false});
		$.ajax({
			...
			url: 'save',
			data: {
				id:id, title: title, dataPost:dataPost
			},
			success: function(data) { 
